...
document::lend(f,nlev) { fprintln(f,"</OL ",nlev," >"); }
...
section::codesegment(line) {
	...
	if (isfile(fm[CODE+1][fptr])) {
		...
		fprintln(fm[CODE+1][fptr],"<li><a target=\"content\" href=\"",loc,"#",fname,"\">",fname,"</a></li>");
                        ...
                        if (( (ftemp=findmark(line))!=FEND )) {
							...
							if (1+ftype==CODE && (puboption>=PUBLISH)){
                            	...
                            	if (isfile(fm[1+ftype][fptr]))
                                	if (puboption>=PUBLISH) {
                                        decl loc = buildtype==BOOK ? "" : sprint(prefs[SECTION],output,outext);
										fprintln(fm[1+ftype][fptr],"<li><a target=\"content\" href=\"",loc,"#",figprefix[ftype],fign[ftype],"\">",curtit,"</a></li>");
...
document::readtoc() {
	decl ch,done,line, iprev,nlev,exappended;
    do {
       fscan(tocf,OxScan,&line);
...
//                if (nlev<contents[iprev].level) println("--- ",nlev," ",curx');
    ...
    fprintln(fm[TOC][fptr],"</OL></div></body></html>");  //</details>
  	fclose(fm[TOC][fptr]);
	}
...
//        fprintln(fm[TOC][fptr],"<LI><a href=\"",loc,"\" target=\"content\">",fm[f][fmtitle],"</a></LI>");
		fprintln(fm[TOC][fptr],"<div class=\"accordion-item\"><h2 class=\"accordion-header\" id=\"H",f,"\">");
		fprintln(fm[TOC][fptr],"<button class=\"accordion-button\" type=\"button\" data-bs-toggle=\"collapse\" data-bs-target=\"#C",f,"\" aria-expanded=\"false\" aria-controls=\"C,f,\">",fm[f][fmtitle],"</button></h2>");
		fprintln(fm[TOC][fptr],"<div id=\"C",f,"\" class=\"accordion-collapse collapse\" aria-labelledby=\"H",f,"\" data-bs-parent=\"#lists\"><div class=\"accordion-body\">");
		fprintln(fm[TOC][fptr],"<iframe style=\"position : relative;top : 0%;left : 0%;width: 100%; height:100%\" src=\"",fm[f][fmname]+outext,"\"></iframe>");
		fprintln(fm[TOC][fptr],"</div></div></div>");		
        }
