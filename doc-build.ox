...
countkbs(ss){
	decl kbvals = "1 2 3 4 5 6 7 8 9 0",smxi=sizeof(ss),
		nxti, nxtf, lsti = 0, tstart,tend, tmp, targ = "\"kb\" value=\"", outs="", mxfrag=0,etarg;
	 while(( lsti<smxi && (nxti=strifind(ss[lsti:],targ))!=FEND)) {
		tstart = strfindr(ss[lsti:lsti+nxti],"<");
//		if (tstart<0) println(ss[lsti:lsti+nxti]);
		tend = strfind(ss[lsti+tstart:lsti+nxti]," ");
		etarg = "</"+ss[lsti+tstart+1:lsti+tstart+tend-1]+">";
		nxti+=sizeof(targ)-1;
		outs|=ss[lsti:lsti+nxti];
		++nxti;
//		if (lsti+nxti<0) println(ss[lsti+tstart:lsti+nxti]);
